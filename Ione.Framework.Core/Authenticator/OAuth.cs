...
namespace Ione.Framework.Core.Authenticator
{
    public class OAuth
    {
        ...
        /// <param name="signatureType">The signature type. To use the default values use <see cref="OAuth.SignatureTypes">OAuth.SignatureTypes</see>.</param>
            ...
            out string normalizedUrl, out string normalizedRequestParameters)
        {
            ...
            List<QueryParameter> parametersQuery = GetQueryParameters(url.Query);
            parametersQuery.Sort(new QueryParameterComparer());


            List<QueryParameter> parametersAuth = new List<QueryParameter>
            {
            ...
            if (verifier != null && verifier != "")
                parametersAuth.Add(new QueryParameter(OAuthVerifier, verifier));

            if (!string.IsNullOrEmpty(token))
            {
                parametersAuth.Add(new QueryParameter(OAuthTokenKey, token));
            }

            parametersAuth.Sort(new QueryParameterComparer());


            List<QueryParameter> parameters = new List<QueryParameter>();
            parameters.AddRange(parametersQuery);
            parameters.AddRange(parametersAuth);
            ...
            out string normalizedUrl, out string normalizedRequestParameters)
        {
            ...
            switch (signatureType)
            {
                ...
                case SignatureTypes.HMACSHA1:
                    ...
                    //return Convert.ToBase64String(hashBytes);
                    return UrlEncode(GenerateSignatureUsingHash(signatureBase1, hmacsha1));


                case SignatureTypes.HMACSHA256:
                    string signatureBase256 =
                        GenerateSignatureBase(
                            url, consumerKey, token, tokenSecret,
                    ...
                    return UrlEncode(GenerateSignatureUsingHash(signatureBase256, hmacsha256));
                default:
